<script lang="ts">
  ...
  async function handleJoin() {
    ...
    joinError = null
    try {
      const { inviteUrl } = await getGroupInviteCode(fetch, data.profile.slug)
      window.open(inviteUrl, '_blank', 'noopener,noreferrer')
    } catch {
      joinError = 'Could not load invite link. The link may have been revoked — try again later.'
    } finally {
      joinLoading = false
    }
  }
    ...
    <div class="hero-layout">
      ...
      <div class="hero-text">
        ...
        {#if data.profile.joinable}
          <div class="join-wrap">
            <button class="join-btn" onclick={handleJoin} disabled={joinLoading}>
              {#if joinLoading}
                <svg
                  class="join-icon spin"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2.5"
